...
{
      if (all->ray.side == 0) 
        all->ray.wallX = all->game.gpos_y + all->ray.perpWallDist * all->ray.ray_dir_y;
      else           
        all->ray.wallX = all->game.gpos_x + all->ray.perpWallDist * all->ray.ray_dir_x;
      all->ray.wallX -= floor((all->ray.wallX));
      all->ray.texX = (int)(all->ray.wallX * (double)(64));
      if(all->ray.side == 0 && all->ray.ray_dir_x > 0) all->ray.texX = 64 - all->ray.texX - 1;
      if(all->ray.side == 1 && all->ray.ray_dir_y < 0) all->ray.texX = 64 - all->ray.texX - 1;
