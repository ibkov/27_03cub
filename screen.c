...
void	ft_ver(t_all *s)
{
	...
	x = floor(s->game.gpos_x) + s->ray.v;
	y = s->game.gpos_y + (x - s->game.gpos_x) * (s->ray.y / s->ray.x);
	while ((int)floor(y) > 0 && (int)floor(y) < s->game.map_y)
	{
		if (s->game.map[(int)floor(y)][(int)(x - 1 + s->ray.v)] == '1')
		{
			s->hit.x = x;
			s->hit.y = y;
			s->hit.d = hypot(x - s->game.gpos_x, y - s->game.gpos_y);
			return ;
		}
		x += (2 * s->ray.v - 1);
		y += (2 * s->ray.v - 1) * s->ray.y / s->ray.x;
	}
	s->hit.x = s->game.gpos_x;
	s->hit.y = s->game.gpos_y;
	s->hit.d = 1000000000;
}
