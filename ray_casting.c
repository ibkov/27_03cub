...
void	calc_step(t_all *all)
{
	...
	{
		all->ray.stepX = -1;
		all->ray.sdx = (all->game.gpos_x - all->ray.mapX) * all->ray.deltaDistX;
	}
	else
	{
		all->ray.stepX = 1;
		all->ray.sdx = (all->ray.mapX + 1.0 - all->game.gpos_x) * \
		all->ray.deltaDistX;
	}
	if (all->ray.ray_dir_y < 0)
	{
		all->ray.stepY = -1;
		all->ray.sdy = (all->game.gpos_y - all->ray.mapY) * all->ray.deltaDistY;
	}
	else
	{
		all->ray.stepY = 1;
		all->ray.sdy = (all->ray.mapY + 1.0 - all->game.gpos_y) * \
		all->ray.deltaDistY;
	}
}
...
void	calc_dda(t_all *all)
{
	while (all->ray.hit == 0)
	{
		if (all->ray.sdx < all->ray.sdy)
		{
			all->ray.sdx += all->ray.deltaDistX;
			all->ray.mapX += all->ray.stepX;
			all->ray.side = 0;
		}
		else
		{
			all->ray.sdy += all->ray.deltaDistY;
...
{
	if (all->ray.side == 0)
		all->ray.perpWallDist = (all->ray.mapX - all->game.gpos_x + \
		(1 - all->ray.stepX) / 2) / all->ray.ray_dir_x;
	else
		all->ray.perpWallDist = (all->ray.mapY - all->game.gpos_y + \
		(1 - all->ray.stepY) / 2) / all->ray.ray_dir_y;
	...
	if (all->ray.drawEnd >= all->ray.h)
		all->ray.drawEnd = all->ray.h - 1;
}
...
int	screen_ray(t_all *all)
{
	...
	all->img.addr = (int *)mlx_get_data_addr(all->img.img_ptr, &all->img.bits_per_pixel, \
	...
	draw_sprite(all, 0);
	mlx_put_image_to_window(all->mlx.mlx_ptr, all->win.win_ptr, \
	all->img.img_ptr, 0, 0);
	return (SUCCESS);
}
